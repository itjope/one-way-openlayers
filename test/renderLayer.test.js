...
describe("Render layer", function() {
  it("Should add layers", () => {
    ...
    expect(oneWayOpenLayers.map.getLayers().getArray().length).toEqual(2)
  })
  ...
  it("Should not create new layers if layer props are equal", () => {
    ...
    oneWayOpenLayers.render(cloneDeep(xyzConfig))
    expect(oneWayOpenLayers.map.getLayers().getArray().length).toEqual(1)
  })

  it("Should remove layers", () => {
    const oneWayOpenLayers = OneWayOpenLayers()
    oneWayOpenLayers.render(cloneDeep(threeLayersConfig))
    expect(oneWayOpenLayers.map.getLayers().getArray().length).toEqual(3)

    oneWayOpenLayers.render({
      view: cloneDeep(xyzConfig.view),
      layers: []
    })
    expect(oneWayOpenLayers.map.getLayers().getArray().length).toEqual(0)
  })
  ...
  it("Should remove a specific layer", () => {
    ...
    expect(oneWayOpenLayers.map.getLayers().getArray().length).toEqual(2)
    expect(oneWayOpenLayers.map.getLayers().item(0).get('name')).toEqual('Layer1')
    expect(oneWayOpenLayers.map.getLayers().item(1).get('name')).toEqual('Layer3')
  })
  ...
  it("Should update a layer", () => {
    ...
    expect(oneWayOpenLayers.map.getLayers().item(1).get('name')).toEqual('Layer2')

    const config2 = cloneDeep(threeLayersConfig)

    config2.layers[1].name = 'Layer2Updated'
    oneWayOpenLayers.render(config2)
    expect(oneWayOpenLayers.map.getLayers().item(1).get('name')).toEqual('Layer2Updated')
  })
